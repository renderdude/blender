...
template<Parameter_Type PT> struct Parameter_Type_Traits {};

class Parameter_Dictionary;
using Vector_Dictionary = std::pair<std::string, std::vector<Parameter_Dictionary>>;
using Mapped_Parameter_Dictionary = std::unordered_map<std::string, Parameter_Dictionary>;
using Mapped_Vector_Dictionary = std::map<std::string, std::vector<Parameter_Dictionary>>;
...
class Parameter_Dictionary {
 public:
  ...
  Parameter_Dictionary(Parsed_Parameter_Vector params0, const Parsed_Parameter_Vector &params1);
  ///@}
  ...
  const Parsed_Parameter_Vector &get_parameter_vector() const
  {
    return params;
  }
  ...
  size_t size()
  {
    size_t result = sizeof(Parameter_Dictionary);
  ...
  // created
  void push_back(Parsed_Parameter *param)
  {
 ...
 private:
  ...
  vector<Return_Type> lookup_array(
  ...
  static std::string to_parameter_definition(const Parsed_Parameter *p, int indentCount);
